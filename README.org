...
*** Emacs 26.1+ with dynamic modules support. Need to add ~--with-modules~ option when compiling it manually.

**** Linux
Emacs included in major linux distributions has dynamic module support enabled by default.

**** MacOS
On MacOS, it is recommended to install ~emacs-plus~ or ~emacs-mac~ for better compatibility.
...
*** *NOTE*: emacs-rime will build dynamic module automatically, hence ~make~ and ~gcc~ is required.
...
*NOTE*: If you build Emacs by yourself and does not install to standard
location,
