...
Required module `emacs-module.h` is included in package `emacs-devel`

#+begin_src bash
  sudo dnf install emacs-devel
#+end_src
...
Required module `emacs-module.h` is included in package `emacs-el`

#+begin_src bash
  sudo dnf install emacs-el
#+end_src
...
To build librime-emacs automatically, you can install build dependencies through [[https://scoop.sh][scoop]].
...
If you are using a system based on RPM, you will need to install extra dependencies. For Fedora, you will need `emacs-devel`; for openSUSE, you will need `emacs-el`.
